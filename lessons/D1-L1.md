# D1-L1: What Does This System Actually Do?

**Day 1: You Just Inherited an AI Product** | ~25 min
...
Pick any row from the dataset. Read through each column value for that row and narrate what happened:

1. **What changed?** (change_type, the relevant old/new pair)
2. **What context did the system have?** (last_verified_price, price_markup_pct)
3. **What did the AI decide?** (llm_decision, llm_confidence, llm_reasoning)
4. **What did the human decide?** (human_decision)
5. **Did they agree?** (human_agrees_with_llm)
...
Your engineering lead asks: "What questions do you have about the system?" Based on your column mapping and row trace, write 5 questions — each targeting a different pipeline stage:

1. **Input parsing:** "___"
2. **Context retrieval:** "___"
3. **LLM reasoning:** "___"
4. **Output generation:** "___"
5. **Human review:** "___"

These questions become your evaluation investigation roadmap. A PM who asks sharp questions about each stage will uncover problems faster than one who only looks at the aggregate accuracy number.
